...
let mockSessionRetrieveFn;
let mockPaymentIntentsRetrieveFn;

jest.mock('stripe', () => {
    // Create the actual mock functions inside the factory
    // and assign them to the outer variables.
    ...
    const mockStripeModule = jest.fn().mockImplementation(() => ({
        checkout: {
            sessions: {
                retrieve: mockSessionRetrieveFn,
            },
        },
        paymentIntents: {
...
describe('Payment API - /api/payments/verify-session', () => {
    ...
    afterEach(async () => {
        ...
        // This is a bit broad, but necessary for isolation if tests create these.
        // A more targeted cleanup based on IDs collected during tests is better.
