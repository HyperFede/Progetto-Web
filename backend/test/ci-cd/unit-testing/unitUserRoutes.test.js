...
describe('User API Unit Tests', () => {
    ...
    describe('GET /api/users/notdeleted', () => {
        ...
        describe('PUT /api/users/:id', () => {
            test('Cliente should update their own profile successfully (Self permission)', async () => {
                ...
                // Set the authenticated user to be a Cliente with the ID we are updating
                ...
                // Mock the initial SELECT query to find the user
                pool.query.mockResolvedValue({
                    rows: [{ ...mockAuthenticatedUser, tipologia: 'Cliente' }]
                });

                // Mock the UPDATE query's RETURNING result
                pool.query.mockResolvedValue({
                    rows: [{ ...mockAuthenticatedUser, ...updateData }]
                });
                ...
                expect(res.statusCode).toBe(200);
                expect(res.body.idutente).toBe(mockUserId);
                expect(res.body.nome).toBe('SelfUpdated');
            });

            test('Artigiano should update their own profile successfully with piva and artigianodescrizione', async () => {
                const mockUserId = 777;
                const updateData = {
                    piva: '11223344556',
                    artigianodescrizione: 'A new description for the artisan',
                };

                // Set the authenticated user to be an Artigiano with the ID we are updating
                mockAuthenticatedUser = {
                    idutente: mockUserId,
                    tipologia: 'Artigiano',
                    username: 'selfartigiano',
                    piva: '12345678901',
                    artigianodescrizione: 'Old description'
                };

                // Mock the initial SELECT query to find the user
                pool.query.mockResolvedValueOnce({
                    rows: [{ ...mockAuthenticatedUser }]
                });
                ...
                expect(res.statusCode).toBe(200);
                expect(res.body.idutente).toBe(mockUserId);
                expect(res.body.piva).toBe(updateData.piva);
                expect(res.body.artigianodescrizione).toBe(updateData.artigianodescrizione);
            });
        });
    });
});